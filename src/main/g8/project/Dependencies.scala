...
object Dependencies {
  ...
  lazy val gatlingPicatinny: Seq[ModuleID] = Seq("io.cosmospf" %% "gatling-picatinny" % "$gatling_picatinny_version$")
  lazy val janino: Seq[ModuleID]           = Seq("org.codehaus.janino" % "janino" % "3.1.8")
  $if(amqpPlugin.truthy) $
  lazy val amqpPlugin: Seq[ModuleID]       = Seq("io.cosmospf" %% "gatling-amqp-plugin" % "0.8.0")
  $endif$
  $if(kafkaPlugin.truthy) $
  lazy val kafkaPlugin: Seq[ModuleID]      = Seq("io.cosmospf" %% "gatling-kafka-plugin" % "0.9.0")
  ...
  lazy val jdbcPlugin: Seq[ModuleID]       = Seq("io.cosmospf" %% "gatling-jdbc-plugin" % "0.7.0")
